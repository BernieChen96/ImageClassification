...
public class WekaTest {
    @Test
    public void startWekaTest() {
        Classifier m_classifier = new J48();
        File inputFile = new File("G:\\数据挖掘平台\\weka\\data\\cpu.with.vendor.arff");//训练语料文件
        ArffLoader atf = new ArffLoader();
        try {
            atf.setFile(inputFile);
            Instances instancesTrain = atf.getDataSet(); // 读入训练文件
            inputFile = new File("G:\\数据挖掘平台\\weka\\data\\cpu.with.vendor.arff");//测试语料文件
            atf.setFile(inputFile);
